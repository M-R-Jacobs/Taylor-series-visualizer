...
The $nth$-degree Taylor polynomial $P_n(x)$ is just the partial sum up through degree $n$ - a finite polynomial approximation to $f$.
...
Integrate by parts on the integral, with $u=f'(t)$ and $dv=-dt$ so that $v = -(x-t)$ rather than $v = t$. This way after evaluating at the bounds the result takes the same form as the Taylor/power series terms - powers of $(x-a)$ with factorial denominators:
